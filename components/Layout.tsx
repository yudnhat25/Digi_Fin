...
interface LayoutProps {
  ...
  onLogout: () => void;
  activeTab: 'dashboard' | 'competition' | 'account';
  setActiveTab: (tab: 'dashboard' | 'competition' | 'account') => void;
}
          ...
          <ul className="space-y-3 px-4">
            ...
            <li>
              <button
                ...
                className={`w-full flex items-center gap-4 px-5 py-3 rounded-2xl transition-all relative ${activeTab === 'account' ? 'bg-emerald-500/10 text-emerald-400 font-black shadow-[inset_0_0_10px_rgba(16,185,129,0.1)]' : 'text-slate-500 hover:bg-slate-800/50 hover:text-slate-100'}`}
              >
                <User size={20} />
                My Account
              </button>
